...
namespace HRM.Repository
{
	...
	public abstract partial class PayrollCycleRepositoryBase : Repository, IPayrollCycleRepositoryBase 
	{
        
        public PayrollCycleRepositoryBase()
        {   
			...
			this.SearchColumns.Add("Month",new SearchColumn(){Name="Month",Title="Month",SelectClause="Month",WhereClause="AllRecords.Month",DataType="System.Int32?",IsForeignColumn=false,PropertyName="Month",IsAdvanceSearchColumn = false,IsBasicSearchColumm = false});
			this.SearchColumns.Add("Year",new SearchColumn(){Name="Year",Title="Year",SelectClause="Year",WhereClause="AllRecords.Year",DataType="System.Int32?",IsForeignColumn=false,PropertyName="Year",IsAdvanceSearchColumn = false,IsBasicSearchColumm = false});
        ...
        public virtual Dictionary<string, string> GetPayrollCycleBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetPayrollCycleAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetPayrollCycleSelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "[PayrollCycle]."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",[PayrollCycle]."+keyValuePair.Key;
		...
		public virtual PayrollCycle GetPayrollCycle(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetPayrollCycleSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [PayrollCycle] with (nolock)  where ID=@ID ";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return PayrollCycleFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<PayrollCycle> GetPayrollCycleByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetPayrollCycleSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [PayrollCycle] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<PayrollCycle>(ds,PayrollCycleFromDataRow);
		}

		public virtual List<PayrollCycle> GetAllPayrollCycle(string SelectClause=null)
		{
		...
		public virtual List<PayrollCycle> GetPagedPayrollCycle(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            count=GetPayrollCycleCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [PayrollCycle] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetPayrollCycleSelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [PayrollCycle] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [PayrollCycle].[ID] = PageIndex.[ID] 
		...
		private int GetPayrollCycleCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM PayrollCycle as AllRecords  ";
			else
				sql = "SELECT Count(*) FROM PayrollCycle as AllRecords  where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(PayrollCycle))]
		public virtual PayrollCycle InsertPayrollCycle(PayrollCycle entity)
		{

			PayrollCycle other=new PayrollCycle();
			other = entity;
...
, @IsFinal);
		...
		[MOLog(AuditOperations.Update,typeof(PayrollCycle))]
		public virtual PayrollCycle UpdatePayrollCycle(PayrollCycle entity)
		{
	...
	, [IsFinal]=@IsFinal 
		...
		[MOLog(AuditOperations.Update,typeof(PayrollCycle))]
		public virtual PayrollCycle UpdatePayrollCycleByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{

			List<SqlParameter> parameterArray = new List<SqlParameter>();
			string sql="Update [PayrollCycle] set ";
		...
		public virtual bool DeletePayrollCycle(System.Int32 Id)
		{

			string sql="delete from [PayrollCycle] where ID=@ID";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			var identity=SqlHelper.ExecuteNonQuery(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			return (Convert.ToInt32(identity))==1? true: false;
		}

		[MOLog(AuditOperations.Delete,typeof(PayrollCycle))]
		public virtual PayrollCycle DeletePayrollCycle(PayrollCycle entity)
		{
			this.DeletePayrollCycle(entity.Id);
			return entity;
		}


		public virtual PayrollCycle PayrollCycleFromDataRow(DataRow dr)
		{
			if(dr==null) return null;
			PayrollCycle entity=new PayrollCycle();
			...
			if (dr.Table.Columns.Contains("Month"))
			{
			entity.Month = dr["Month"]==DBNull.Value?(System.Int32?)null:(System.Int32?)dr["Month"];
			}
			if (dr.Table.Columns.Contains("Year"))
			{
			entity.Year = dr["Year"]==DBNull.Value?(System.Int32?)null:(System.Int32?)dr["Year"];
			}
