...
namespace HRM.Repository
{
	...
	public abstract partial class StateRepositoryBase : Repository, IStateRepositoryBase 
	{
		...
		public virtual List<State> GetStateByCountryId(System.Int32? CountryId,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetStateSelectClause():(string.Format("Select [{0}] ",SelectClause));
			sql+="from [State] with (nolock)  where CountryID=@CountryID  ";
		...
		public virtual State GetState(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetStateSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [State] with (nolock)  where ID=@ID ";
		...
		public virtual List<State> GetStateByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetStateSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [State] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<State>(ds,StateFromDataRow);
		}

		public virtual List<State> GetAllState(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetStateSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [State] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<State>(ds, StateFromDataRow);
		}

		public virtual List<State> GetPagedState(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [State] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetStateSelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [State] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [State].[ID] = PageIndex.[ID] 
		...
		public virtual State UpdateState(State entity)
		{
			...
			State other = GetState(entity.Id);
			if (entity.Equals(other)) return entity;
			string sql=@"Update [State] set  [Name]=@Name
							, [CountryID]=@CountryID
							, [IsActive]=@IsActive
			...
			return GetState(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(State))]
		public virtual State UpdateStateByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{
		...
		public virtual bool DeleteState(System.Int32 Id)
		{
