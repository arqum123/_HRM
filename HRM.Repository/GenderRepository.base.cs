...
namespace HRM.Repository
{
	...
	public abstract partial class GenderRepositoryBase : Repository, IGenderRepositoryBase 
	{
        
        public GenderRepositoryBase()
        {   
		...
		public virtual List<SearchColumn> GetGenderSearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetGenderBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetGenderAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetGenderSelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "[Gender]."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",[Gender]."+keyValuePair.Key;
		...
		public virtual Gender GetGender(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetGenderSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Gender] with (nolock)  where ID=@ID ";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return GenderFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<Gender> GetGenderByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetGenderSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [Gender] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Gender>(ds,GenderFromDataRow);
		}

		public virtual List<Gender> GetAllGender(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetGenderSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Gender] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Gender>(ds, GenderFromDataRow);
		}

		public virtual List<Gender> GetPagedGender(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            count=GetGenderCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [Gender] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetGenderSelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [Gender] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [Gender].[ID] = PageIndex.[ID] 
		...
		private int GetGenderCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM Gender as AllRecords  ";
			else
				sql = "SELECT Count(*) FROM Gender as AllRecords  where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(Gender))]
		public virtual Gender InsertGender(Gender entity)
		{
			...
			{
				string sql=@"Insert into [Gender] ( [Name]
				,[IsActive]
				,[CreationDate]
				,[UpdateDate]
				,[UpdateBy]
				...
				return GetGender(Convert.ToInt32(identity));
			}
			return entity;
		}

		[MOLog(AuditOperations.Update,typeof(Gender))]
		public virtual Gender UpdateGender(Gender entity)
		{

			if (entity.IsTransient()) return entity;
			Gender other = GetGender(entity.Id);
			if (entity.Equals(other)) return entity;
			string sql=@"Update [Gender] set  [Name]=@Name
							, [IsActive]=@IsActive
			...
			return GetGender(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(Gender))]
		public virtual Gender UpdateGenderByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{
		...
		public virtual bool DeleteGender(System.Int32 Id)
		{
		...
		[MOLog(AuditOperations.Delete,typeof(Gender))]
		public virtual Gender DeleteGender(Gender entity)
		{
			this.DeleteGender(entity.Id);
			return entity;
		}


		public virtual Gender GenderFromDataRow(DataRow dr)
		{
