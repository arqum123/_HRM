...
namespace HRM.Repository
{
	...
	public abstract partial class ShiftOffDayRepositoryBase : Repository, IShiftOffDayRepositoryBase 
	{
        
        public ShiftOffDayRepositoryBase()
        {   
			...
			this.SearchColumns.Add("OffDayOfWeek",new SearchColumn(){Name="OffDayOfWeek",Title="OffDayOfWeek",SelectClause="OffDayOfWeek",WhereClause="AllRecords.OffDayOfWeek",DataType="System.Int32?",IsForeignColumn=false,PropertyName="OffDayOfWeek",IsAdvanceSearchColumn = false,IsBasicSearchColumm = false});
		...
		public virtual List<SearchColumn> GetShiftOffDaySearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetShiftOffDayBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetShiftOffDayAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetShiftOffDaySelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "[ShiftOffDay]."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",[ShiftOffDay]."+keyValuePair.Key;
		...
		public virtual List<ShiftOffDay> GetShiftOffDayByShiftId(System.Int32? ShiftId,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetShiftOffDaySelectClause():(string.Format("Select [{0}] ",SelectClause));
			sql+="from [ShiftOffDay] with (nolock)  where ShiftID=@ShiftID  ";
			SqlParameter parameter=new SqlParameter("@ShiftID",ShiftId);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<ShiftOffDay>(ds,ShiftOffDayFromDataRow);
		}

		public virtual ShiftOffDay GetShiftOffDay(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetShiftOffDaySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [ShiftOffDay] with (nolock)  where ID=@ID ";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return ShiftOffDayFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<ShiftOffDay> GetShiftOffDayByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetShiftOffDaySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [ShiftOffDay] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<ShiftOffDay>(ds,ShiftOffDayFromDataRow);
		}

		public virtual List<ShiftOffDay> GetAllShiftOffDay(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetShiftOffDaySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [ShiftOffDay] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<ShiftOffDay>(ds, ShiftOffDayFromDataRow);
		}

		public virtual List<ShiftOffDay> GetPagedShiftOffDay(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            count=GetShiftOffDayCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [ShiftOffDay] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetShiftOffDaySelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [ShiftOffDay] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [ShiftOffDay].[ID] = PageIndex.[ID] 
		...
		private int GetShiftOffDayCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM ShiftOffDay as AllRecords  ";
			else
				sql = "SELECT Count(*) FROM ShiftOffDay as AllRecords  where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(ShiftOffDay))]
		public virtual ShiftOffDay InsertShiftOffDay(ShiftOffDay entity)
		{

			ShiftOffDay other=new ShiftOffDay();
			other = entity;
			if(entity.IsTransient())
			{
				string sql=@"Insert into [ShiftOffDay] ( [ShiftID]
				,[OffDayOfWeek]
				,[EffectiveDate]
				,[RetiredDate]
				,[CreationDate]
				...
				SqlParameter[] parameterArray=new SqlParameter[]{
					...
					, new SqlParameter("@OffDayOfWeek",entity.OffDayOfWeek ?? (object)DBNull.Value)
				...
				return GetShiftOffDay(Convert.ToInt32(identity));
			}
			return entity;
		}

		[MOLog(AuditOperations.Update,typeof(ShiftOffDay))]
		public virtual ShiftOffDay UpdateShiftOffDay(ShiftOffDay entity)
		{

			if (entity.IsTransient()) return entity;
			ShiftOffDay other = GetShiftOffDay(entity.Id);
			if (entity.Equals(other)) return entity;
			string sql=@"Update [ShiftOffDay] set  [ShiftID]=@ShiftID
			...
			return GetShiftOffDay(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(ShiftOffDay))]
		public virtual ShiftOffDay UpdateShiftOffDayByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{

			List<SqlParameter> parameterArray = new List<SqlParameter>();
			string sql="Update [ShiftOffDay] set ";
			...
			return GetShiftOffDay(Id);
		}

		public virtual bool DeleteShiftOffDay(System.Int32 Id)
		{

			string sql="delete from [ShiftOffDay] where ID=@ID";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			var identity=SqlHelper.ExecuteNonQuery(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			return (Convert.ToInt32(identity))==1? true: false;
		}

		[MOLog(AuditOperations.Delete,typeof(ShiftOffDay))]
		public virtual ShiftOffDay DeleteShiftOffDay(ShiftOffDay entity)
		{
			this.DeleteShiftOffDay(entity.Id);
			return entity;
		}


		public virtual ShiftOffDay ShiftOffDayFromDataRow(DataRow dr)
		{
			if(dr==null) return null;
			ShiftOffDay entity=new ShiftOffDay();
			...
			if (dr.Table.Columns.Contains("OffDayOfWeek"))
			{
			entity.OffDayOfWeek = dr["OffDayOfWeek"]==DBNull.Value?(System.Int32?)null:(System.Int32?)dr["OffDayOfWeek"];
			}
