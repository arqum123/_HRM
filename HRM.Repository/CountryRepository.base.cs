...
namespace HRM.Repository
{
	...
	public abstract partial class CountryRepositoryBase : Repository, ICountryRepositoryBase 
	{
		...
		public virtual Country GetCountry(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetCountrySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Country] with (nolock)  where ID=@ID ";
		...
		public virtual List<Country> GetCountryByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetCountrySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [Country] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Country>(ds,CountryFromDataRow);
		}

		public virtual List<Country> GetAllCountry(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetCountrySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Country] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Country>(ds, CountryFromDataRow);
		}

		public virtual List<Country> GetPagedCountry(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [Country] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetCountrySelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [Country] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [Country].[ID] = PageIndex.[ID] 
		...
		public virtual Country UpdateCountry(Country entity)
		{
			...
			Country other = GetCountry(entity.Id);
			...
			return GetCountry(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(Country))]
		public virtual Country UpdateCountryByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{
		...
		public virtual bool DeleteCountry(System.Int32 Id)
		{
