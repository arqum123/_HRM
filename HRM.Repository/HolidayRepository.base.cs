...
namespace HRM.Repository
{
	...
	public abstract partial class HolidayRepositoryBase : Repository, IHolidayRepositoryBase 
	{
        
        public HolidayRepositoryBase()
        {   
		...
		public virtual List<SearchColumn> GetHolidaySearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetHolidayBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetHolidayAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetHolidaySelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "[Holiday]."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",[Holiday]."+keyValuePair.Key;
		...
		public virtual Holiday GetHoliday(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetHolidaySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Holiday] with (nolock)  where ID=@ID ";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return HolidayFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<Holiday> GetHolidayByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetHolidaySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [Holiday] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Holiday>(ds,HolidayFromDataRow);
		}

		public virtual List<Holiday> GetAllHoliday(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetHolidaySelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Holiday] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Holiday>(ds, HolidayFromDataRow);
		}

		public virtual List<Holiday> GetPagedHoliday(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            count=GetHolidayCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [Holiday] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetHolidaySelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [Holiday] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [Holiday].[ID] = PageIndex.[ID] 
		...
		private int GetHolidayCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM Holiday as AllRecords  ";
			else
				sql = "SELECT Count(*) FROM Holiday as AllRecords  where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(Holiday))]
		public virtual Holiday InsertHoliday(Holiday entity)
		{

			Holiday other=new Holiday();
			other = entity;
			if(entity.IsTransient())
			{
				string sql=@"Insert into [Holiday] ( [Name]
				,[Date]
				,[Detail]
				,[IsActive]
				,[CreationDate]
				...
				return GetHoliday(Convert.ToInt32(identity));
			}
			return entity;
		}

		[MOLog(AuditOperations.Update,typeof(Holiday))]
		public virtual Holiday UpdateHoliday(Holiday entity)
		{

			if (entity.IsTransient()) return entity;
			Holiday other = GetHoliday(entity.Id);
			if (entity.Equals(other)) return entity;
			string sql=@"Update [Holiday] set  [Name]=@Name
							, [Date]=@Date
							, [Detail]=@Detail
							, [IsActive]=@IsActive
			...
			return GetHoliday(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(Holiday))]
		public virtual Holiday UpdateHolidayByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{
		...
		public virtual bool DeleteHoliday(System.Int32 Id)
		{
		...
		[MOLog(AuditOperations.Delete,typeof(Holiday))]
		public virtual Holiday DeleteHoliday(Holiday entity)
		{
			this.DeleteHoliday(entity.Id);
			return entity;
		}


		public virtual Holiday HolidayFromDataRow(DataRow dr)
		{
