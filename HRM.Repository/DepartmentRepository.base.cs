...
namespace HRM.Repository
{
	...
	public abstract partial class DepartmentRepositoryBase : Repository, IDepartmentRepositoryBase 
	{
        
        public DepartmentRepositoryBase()
        {   
		...
		public virtual List<SearchColumn> GetDepartmentSearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetDepartmentBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetDepartmentAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetDepartmentSelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "[Department]."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",[Department]."+keyValuePair.Key;
		...
		public virtual Department GetDepartment(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetDepartmentSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Department] with (nolock)  where ID=@ID ";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return DepartmentFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<Department> GetDepartmentByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetDepartmentSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [Department] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Department>(ds,DepartmentFromDataRow);
		}

		public virtual List<Department> GetAllDepartment(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetDepartmentSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [Department] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<Department>(ds, DepartmentFromDataRow);
		}

		public virtual List<Department> GetPagedDepartment(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            count=GetDepartmentCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [Department] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetDepartmentSelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [Department] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [Department].[ID] = PageIndex.[ID] 
		...
		private int GetDepartmentCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM Department as AllRecords  ";
			else
				sql = "SELECT Count(*) FROM Department as AllRecords  where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(Department))]
		public virtual Department InsertDepartment(Department entity)
		{
			...
			if(entity.IsTransient())
			{
				...
				return GetDepartment(Convert.ToInt32(identity));
			}
			return entity;
		}

		[MOLog(AuditOperations.Update,typeof(Department))]
		public virtual Department UpdateDepartment(Department entity)
		{

			if (entity.IsTransient()) return entity;
			Department other = GetDepartment(entity.Id);
			...
			return GetDepartment(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(Department))]
		public virtual Department UpdateDepartmentByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{
		...
		public virtual bool DeleteDepartment(System.Int32 Id)
		{
		...
		[MOLog(AuditOperations.Delete,typeof(Department))]
		public virtual Department DeleteDepartment(Department entity)
		{
			this.DeleteDepartment(entity.Id);
			return entity;
		}


		public virtual Department DepartmentFromDataRow(DataRow dr)
		{
