...
namespace HRM.Repository
{
	...
	public abstract partial class UserTypeRepositoryBase : Repository, IUserTypeRepositoryBase 
	{
        
        public UserTypeRepositoryBase()
        {   
		...
		public virtual List<SearchColumn> GetUserTypeSearchColumns()
        {
        ...
        public virtual Dictionary<string, string> GetUserTypeBasicSearchColumns()
        {
        ...
        public virtual List<SearchColumn> GetUserTypeAdvanceSearchColumns()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
        ...
        public virtual string GetUserTypeSelectClause()
        {
            List<SearchColumn> searchColumns = new List<SearchColumn>();
            ...
            foreach (KeyValuePair<string, SearchColumn> keyValuePair in this.SearchColumns)
            {
                if (!keyValuePair.Value.IgnoreForDefaultSelect)
                {
                	...
                	else
                	{
                    	if (string.IsNullOrEmpty(selectQuery))
                    	{
                        	selectQuery =  "[UserType]."+keyValuePair.Key;
                    	}
                    	else
                    	{
                        	selectQuery += ",[UserType]."+keyValuePair.Key;
		...
		public virtual UserType GetUserType(System.Int32 Id,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetUserTypeSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [UserType] with (nolock)  where ID=@ID ";
			SqlParameter parameter=new SqlParameter("@ID",Id);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,new SqlParameter[] { parameter });
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count != 1) return null;
			return UserTypeFromDataRow(ds.Tables[0].Rows[0]);
		}

		public virtual List<UserType> GetUserTypeByKeyValue(string Key,string Value,Operands operand,string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetUserTypeSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+= string.Format("from [UserType] with (nolock)  where {0} {1} {2} ",Key,operand.ToOperandString(),Value);
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<UserType>(ds,UserTypeFromDataRow);
		}

		public virtual List<UserType> GetAllUserType(string SelectClause=null)
		{

			string sql=string.IsNullOrEmpty(SelectClause)?GetUserTypeSelectClause():(string.Format("Select {0} ",SelectClause));
			sql+="from [UserType] with (nolock)  ";
			DataSet ds=SqlHelper.ExecuteDataset(this.ConnectionString,CommandType.Text,sql,null);
			if (ds == null || ds.Tables.Count != 1 || ds.Tables[0].Rows.Count == 0) return null;
			return CollectionFromDataSet<UserType>(ds, UserTypeFromDataRow);
		}

		public virtual List<UserType> GetPagedUserType(string orderByClause, int pageSize, int startIndex,out int count, List<SearchColumn> searchColumns,string SelectClause=null)
		{
            ...
            count=GetUserTypeCount(whereClause, searchColumns);
			if(count>0)
			{
            ...
            tempsql += " [ID] ";
            tempsql += " FROM [UserType] AllRecords with (NOLOCK)";
            ...
            string pagedResultsSql =
                (string.IsNullOrEmpty(SelectClause)? GetUserTypeSelectClause():(string.Format("Select {0} ",SelectClause)))+@" FROM [UserType] , #PageIndex PageIndex WHERE ";
            pagedResultsSql += " PageIndex.IndexId > " + PageLowerBound.ToString(); 
            pagedResultsSql += @" AND [UserType].[ID] = PageIndex.[ID] 
		...
		private int GetUserTypeCount(string whereClause, List<SearchColumn> searchColumns)
		{

			string sql=string.Empty;
			if(string.IsNullOrEmpty(whereClause))
				sql = "SELECT Count(*) FROM UserType as AllRecords  ";
			else
				sql = "SELECT Count(*) FROM UserType as AllRecords  where  " +whereClause;
			var rowCount = SqlHelper.ExecuteScalar(this.ConnectionString, CommandType.Text, sql, GetSQLParamtersBySearchColumns(searchColumns));
			return rowCount == DBNull.Value ? 0 :(int)rowCount;
		}

		[MOLog(AuditOperations.Create,typeof(UserType))]
		public virtual UserType InsertUserType(UserType entity)
		{

			UserType other=new UserType();
			other = entity;
			if(entity.IsTransient())
			{
				string sql=@"Insert into [UserType] ( [Name]
				,[IsActive]
				,[CreationDate]
				,[UpdateDate]
				,[UpdateBy]
				...
				return GetUserType(Convert.ToInt32(identity));
			}
			return entity;
		}

		[MOLog(AuditOperations.Update,typeof(UserType))]
		public virtual UserType UpdateUserType(UserType entity)
		{

			if (entity.IsTransient()) return entity;
			UserType other = GetUserType(entity.Id);
			if (entity.Equals(other)) return entity;
			string sql=@"Update [UserType] set  [Name]=@Name
							, [IsActive]=@IsActive
			...
			return GetUserType(entity.Id);
		}

		[MOLog(AuditOperations.Update,typeof(UserType))]
		public virtual UserType UpdateUserTypeByKeyValue (Dictionary<string, string> UpdateKeyValue, System.Int32 Id)
		{
		...
		public virtual bool DeleteUserType(System.Int32 Id)
		{
		...
		[MOLog(AuditOperations.Delete,typeof(UserType))]
		public virtual UserType DeleteUserType(UserType entity)
		{
			this.DeleteUserType(entity.Id);
			return entity;
		}


		public virtual UserType UserTypeFromDataRow(DataRow dr)
		{
